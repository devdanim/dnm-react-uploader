...
export default class Uploader extends React.Component {
    ...
    componentDidUpdate(prevProps) {
        ...
        if (this.props.fetching && !prevProps.fetching && this.props.src && prevProps.src === this.props.src && this.state.loaded) cl(
            '=====',
            1,
                    ...
                    if (this.state.loaded && this.state.mounted && this.props.imageCrop && this.zone) {
                        ...
                        media = (
                            <div style={{
                                ...
                                backgroundImage: this.state.loaded ? `url(${this.props.src}${this.props.src.match(/^http/) ? `${this.props.src.includes('?') ? '&' : '?'}xCssCors=1` : ''})` : null, // this is no-CORS request, we therefore need to be sure the cached response (e.g. Chrome) has been a CORS one (see the <img /> below) before display
                                position: 'relative',
                                width: '100%',
                                height: '100%',
                            }}>
